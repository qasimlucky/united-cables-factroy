import React,{ useEffect, useState } from "react";
import axios from "axios";
import Sidebar from "../components/Sidebar";
import Navbar from "../components/Navbar";
import Footer from "../components/Footer";
import { Link,useNavigate } from 'react-router-dom';

function PurchaseList() {
  const [data, setData] = useState([{}])

    useEffect(() => {
      axios.get("http://localhost:7000/purchase/get").then(Response =>{
        console.log(Response.data)
          setData(Response.data)
      }).catch(err =>{
        console.log(err)
      })
      },[]);

      let navigate = useNavigate();
      function editSale(sendData){
         console.log("this is send dataaaaa")
          console.log(sendData)
          navigate("/editpurchase",{state:{sendData:sendData}})    
        }

       return (
        <>
      <Navbar/>
      <Sidebar/>
      <div className="main-content">
        <section className="section">
          <div className="row">
            <div className="col-12">
              <div className="card">
                <div className="card-header">
                  <h4>All Purchase</h4>
                  <div className="card-header-form">
                    <Link to="/addpurchase" className="btn btn-success ">
                      + Add Purchase
                    </Link>
                  </div>
                </div>
                <div className="card-header mb-3">
                  <div>
                    <div className="input-group">
                      <input
                        type="text"
                        className="form-control"
                        placeholder="Search"
                      />
                      <div className="input-group-btn ml-1">
                        <button className="btn btn-primary">
                          <i className="fas fa-search" />
                        </button>
                      </div>
                    </div>
                  </div>
                </div>
                <div className="card-body p-0">
                  <div className="table-responsive">
                    <table className="table table-striped">
                      <tbody>
                        <tr className="align-center">
                          <th>Name</th>
                          <th>unit</th>
                          <th>purchase amount</th>
                          <th>categroy</th>
                          <th>Quantity</th>
                          <th>Supplier</th>
                          <th>purchase status</th>
                          <th>Action</th>
                         </tr> 
                         {data && data.map(purchaseDetails => ( 
                          <tr className="align-center">

                          <td>{purchaseDetails.product_name}</td>
                          <td>{purchaseDetails.product_unit}</td>
                          <td>{purchaseDetails.purchase_amount}</td>
                          <td>{purchaseDetails.product_category}</td>
                          <td>{purchaseDetails.product_quantity}</td>
                          <td>{purchaseDetails.supplier}</td>
                          <td>{purchaseDetails.purchase_status}</td>
                          <td>
                            <a  onClick={()=>editSale(purchaseDetails)} className="btn btn-primary" style={{color:"white"}}>
                              Edit
                            </a>
                          </td>
                        </tr>
                           ))} 
                      </tbody>
                    </table>
                    <div >

                      </div>
                  </div>
                </div>
              </div>
            </div>
          </div>
        </section>
      </div>
      <Footer/>
        </>
       );
    }

export default PurchaseList;